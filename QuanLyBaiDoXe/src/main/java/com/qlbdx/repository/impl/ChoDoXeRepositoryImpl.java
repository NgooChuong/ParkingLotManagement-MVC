...
//            session.clear();
    ...
    public List<Long> getDistinctChodoIds(Map<String, String> params) {
        ...
        if (startTime != "" && endTime != "") {
            ...
            Predicate p1 = builder.between(thongTinDangKyJoin.get("thoiGianVoBai"), new Date(), new Date());
            Predicate p2 = builder.between(thongTinDangKyJoin.get("thoiGianRaBai"), new Date(), new Date());
            Predicate p3 = builder.and(
                    builder.lessThan(thongTinDangKyJoin.get("thoiGianVoBai"), new Date()),
                    builder.greaterThan(thongTinDangKyJoin.get("thoiGianRaBai"), new Date())
            );
            // Gộp các điều kiện `OR` lại
            predicates.add(builder.or(p1, p2, p3));
        }
